...
const EditSomeMovie = () => {
  ...
  const ReadData = async()=>{
    try {
    const data =   await axios.get(`http://localhost:3000/api/v1/readmovie/${id}`
    )
...
const UpdateProduct = async(e)=>{
  e.preventDefault()
  try {
    await axios.put(`http://localhost:3000/api/v1/updatemovie/${id}`,{
      name,
      img,
      detelis,
      rating,
  ...
  navigate(`/admin/dashboar/movies-view`)
