...
const ViewSeries = () => {
    ...
    const Product = async()=>{
        const data = await fetch('http://localhost:3000/api/v1/getseries')
...
const DeleteProduct = async(id) =>{
  try {
    const res = await axios.delete(`http://localhost:3000/api/v1/deleteseries/${id}`)
    Product()
  } catch (error) {
    console.log(error)
  }
        ...
        return(
            <tr> 
                ...
                <td>
                    ...
                    <Link to={`/admin/dashboar/series-view-edit/${pd._id}`} className='text-light text-decoration-none '>
                      Edit
                    </Link>
                      </button>
